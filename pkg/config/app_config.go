...
// You can view the default config with `dcli --config`.
...
type UserConfig struct {
	...
	// Stats determines how long dcli will gather container stats for, and
	...
	// Projects allows the user to define a list of projects that dcli will
...
type GraphConfig struct {
	...
	// see all the options available. Alternatively if you go into dcli and
...
type CustomCommand struct {
	...
	// InternalFunction is the name of a function inside dcli that we want to run, as opposed to a command-line command. This is only used internally and can't be configured by the user
...
// AppConfig contains the base configuration fields required for dcli.
type AppConfig struct {
	Debug        bool   `long:"debug" env:"DEBUG" default:"false"`
	Version      string `long:"version" env:"VERSION" default:"unversioned"`
	Commit       string `long:"commit" env:"COMMIT"`
	BuildDate    string `long:"build-date" env:"BUILD_DATE"`
	Name         string `long:"name" env:"NAME" default:"dcli"`
...
func NewAppConfig(name, version, commit, date string, buildSource string, debuggingFlag bool, composeFiles []string, projectDir string, projectName string, showAll bool, showNotRunning bool) (*AppConfig, error) {
	...
	// Merge compose files from config with those detected or provided via flags
	if len(userConfig.DevOS.DefaultComposeFiles) > 0 {
		composeFiles = lo.Uniq(append(composeFiles, userConfig.DevOS.DefaultComposeFiles...))
	}

	// We will pass compose files as individual -f flags to docker compose
	// dynamically in NewCommandObject to avoid polluting commands for other projects.
...
func configDir(projectName string) string {
	// 1. 最高优先级：~/.config/dcli (Linux 风格/开发者偏好)
