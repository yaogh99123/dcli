...
type DockerCommand struct {
	...
	// LocalProjectName is the compose project name for the directory where dcli was launched.
...
func (c *DockerCommand) NewCommandObject(obj CommandObject) CommandObject {
	...
	if obj.Service != nil && obj.Service.ProjectName != "" {
		projectName = obj.Service.ProjectName
	} else if obj.Project != nil && obj.Project.Name != "" {
		projectName = obj.Project.Name
	}

	// Try to find the correct project directory from loaded compose files
	if projectName != "" {
		for _, file := range c.Config.ComposeFiles {
			absPath, _ := filepath.Abs(file)
			// Match if the project name is part of the path or if it's the directory name
			if strings.Contains(absPath, "/"+projectName+"/") || filepath.Base(filepath.Dir(absPath)) == projectName {
	...
	// Fallback to the first compose file's directory if no specific match was found
	if projectDir == "" && len(c.Config.ComposeFiles) > 0 {
		projectDir = filepath.Dir(c.Config.ComposeFiles[0])
	}

	// Only use local -f flags if the service is defined in local compose files
	if obj.Service != nil && obj.Service.IsLocal {
		if len(c.Config.ComposeFiles) > 0 {
			composeFiles = " -f " + strings.Join(c.Config.ComposeFiles, " -f ")
		}
	} else if obj.Service == nil && (projectName == "" || projectName == c.LocalProjectName) {
		// Fallback for project-level commands (e.g. up, down)
		if len(c.Config.ComposeFiles) > 0 {
			composeFiles = " -f " + strings.Join(c.Config.ComposeFiles, " -f ")
		}
	}

	composeCommand := c.Config.UserConfig.CommandTemplates.DockerCompose + composeFiles
	if projectDir != "" {
		composeCommand = fmt.Sprintf("%s --project-directory %s", composeCommand, projectDir)
	}
	...
	if projectName != "" {
		defaultObj.DockerCompose = fmt.Sprintf("%s -p %s", defaultObj.DockerCompose, projectName)
	}
...
// IsProjectScoped reports whether dcli should be scoped to a single
...
func (c *DockerCommand) mergeServices(containerServices []*Service, composeServices []*Service) []*Service {
	...
	// that aren't already covered by compose (matching by name)
	...
	for _, svc := range containerServices {
		if composeServiceNames[svc.Name] {
			// This service is defined in our loaded compose files,
			// so the composeService version (which has IsLocal: true)
			// already covers it. We don't need the container-derived one.
			continue
		}
...
func (c *DockerCommand) GetServices() ([]*Service, error) {
	...
	commandObj := c.NewCommandObject(CommandObject{})
	output, err := c.OSCommand.RunCommandWithOutput(fmt.Sprintf("%s config --format json", commandObj.DockerCompose))
	if err != nil {
		return nil, err
	}
