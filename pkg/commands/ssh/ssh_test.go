	...
	for _, s := range scenarios {
		...
		t.Run(s.testName, func(t *testing.T) {
			...
			tempDir := func(dir string, pattern string) (string, error) {
				...
				assert.Equal(t, "dcli-sshtunnel-", pattern)

				return "/tmp/dcli-ssh-tunnel-12345", nil
			}

			setenv := func(key, value string) error {
				assert.Equal(t, "DOCKER_HOST", key)
				assert.Equal(t, "unix:///tmp/dcli-ssh-tunnel-12345/dockerhost.sock", value)
				return nil
			}

			startCmdCount := 0
			startCmd := func(cmd *exec.Cmd) error {
				assert.EqualValues(t, []string{"ssh", "-L", "/tmp/dcli-ssh-tunnel-12345/dockerhost.sock:/var/run/docker.sock", "192.168.5.178", "-N"}, cmd.Args)
			...
			dialContext := func(ctx context.Context, network string, address string) (io.Closer, error) {
				...
				assert.Equal(t, "/tmp/dcli-ssh-tunnel-12345/dockerhost.sock", address)
